...
import { resolve_database_name, set_current_database } from './context.js';
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
      ...
      if (request.params.name === 'execute_query') {
        const { query, params = {}, database } = request.params.arguments as {
          query: string;
        ...
        const result = await database_client.execute_query(database_name, query, params);
      ...
      if (request.params.name === 'describe_table') {
        ...
        const columns = await database_client.describe_table(database_name, table);
      ...
      if (request.params.name === 'vector_search') {
        const { table, vector_column, query_vector, limit = 10, database } = request.params.arguments as {
          table: string;
