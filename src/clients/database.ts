...
export async function get_database_client(
  ...
  try {
    ...
    throw new TursoApiError(
      `Failed to create client for database ${database_name}: ${(error as Error).message}`,
      500
    );
  }
}
 ...
 */
export async function list_tables(database_name: string): Promise<string[]> {
  try {
    ...
    throw new TursoApiError(
      `Failed to list tables for database ${database_name}: ${(error as Error).message}`,
      500
    );
  }
}
...
export async function execute_query(
  ...
  try {
    ...
    const is_read_only = query.trim().toLowerCase().startsWith('select');
    ...
    throw new TursoApiError(
      `Failed to execute query for database ${database_name}: ${(error as Error).message}`,
      500
    );
  }
}
...
export async function describe_table(
  ...
  try {
    ...
    throw new TursoApiError(
      `Failed to describe table ${table_name} for database ${database_name}: ${(error as Error).message}`,
      500
    );
  }
}
